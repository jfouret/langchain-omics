...
class UnifiedSRAClient(BaseModel):
    ...
    def _make_request(self, url: str, params: Optional[Dict[str, Any]] = None) -> Any:
        ...
        try:
            ...
            self.logger.log(5, f"TRACE: Response status {response.status_code}, content length: {len(response.content) if response.content else 0}")
    ...
    def discover_fields(self, sra_domain: str) -> List[str]:
        ...
        if sra_domain in self.field_cache:
            self.logger.debug(f"Using cached fields for {sra_domain}: {len(self.field_cache[sra_domain])} fields")
        ...
        try:
            ...
            if domains_info:
                ...
                self.logger.debug(f"Discovered {len(retrievable_fields)} retrievable fields for {sra_domain}")
    ...
    def search_domain(self, domain: str, query: str, size: int = 1000) -> List[Dict[str, Any]]:
        ...
        """
        self.logger.debug(f"Searching domain {domain} with query '{query}', size={size}")
        ...
        try:
            ...
            self.logger.debug(f"Domain search returned {len(entries)} entries for {domain}")
            return entries
        except Exception as e:
    ...
    def get_entries_batch(self, domain: str, entry_ids: List[str], 
        ...
        self.logger.debug(f"Getting entries batch for {domain}: {len(entry_ids)} entries")
        ...
        try:
            ...
            self.logger.debug(f"Processed {len(cleaned_entries)} cleaned entries for {domain}")
            return cleaned_entries
        except Exception as e:
            self.logger.debug(f"Get entries batch failed for {domain}: {e}")
            return []
    ...
    def get_entries_paginated(self, domain: str, entry_ids: List[str], 
        ...
        self.logger.debug(f"Getting entries paginated for {domain}: {len(entry_ids)} total entries")
        all_entries = []
        
        # Process in batches of 100
        for i in range(0, len(entry_ids), 100):
            batch_ids = entry_ids[i:i+100]
            self.logger.debug(f"Processing batch from {i} for {domain}: {len(batch_ids)} entries")
    ...
    def get_cross_references_batch(self, domain: str, entry_ids: List[str], target_domain: str, chunk_size: int = 100) -> Dict[str, List[str]]:
    ...
    def get_full_study_data(self, study_accessions: List[str]) -> List[Dict[str, Any]]:
        ...
        self.logger.debug(f"Getting full study data for {len(study_accessions)} studies")
        ...
        study_ids = [s.get("acc") or s.get("id") for s in studies if s.get("acc") or s.get("id")]
        study_to_experiments = self.get_cross_references_batch("sra-study", study_ids, "sra-experiment")
        ...
        self.logger.debug("Step 3: Getting experiment → sample and experiment → run cross-references")
        exp_ids_list = list(all_experiment_ids)
        experiment_to_samples = self.get_cross_references_batch("sra-experiment", exp_ids_list, "sra-sample")
        experiment_to_runs = self.get_cross_references_batch("sra-experiment", exp_ids_list, "sra-run")
        ...
        self.logger.debug(f"Found {len(all_sample_ids)} unique samples and {len(all_run_ids)} unique runs")
        ...
        return self._assemble_nested_structure(
            studies, samples, experiments, runs,
            study_to_experiments, experiment_to_samples, experiment_to_runs
        )
    ...
    def _assemble_nested_structure(self, studies: List[Dict], samples: List[Dict], 
                                  experiments: List[Dict], runs: List[Dict],
                                  study_to_experiments: Dict[str, List[str]],
                                  experiment_to_samples: Dict[str, List[str]],
                                  experiment_to_runs: Dict[str, List[str]]) -> List[Dict[str, Any]]:
        """Assemble flat data into nested Study -> Experiment -> Sample + Runs structure using provided mappings.
        ...
        """
        self.logger.debug(f"Assembling nested structure: {len(studies)} studies, {len(experiments)} experiments, {len(samples)} samples, {len(runs)} runs")
        ...
        self.logger.debug(f"Built lookup maps")
        ...
        for study_acc, study_data in study_map.items():
            ...
            self.logger.debug(f"Study {study_acc} has {len(study_experiments)} experiments")
            ...
            for exp_acc in study_experiments:
                if exp_acc in experiment_map:
                    ...
                    self.logger.debug(f"Experiment {exp_acc} has {len(exp_runs)} runs and sample: {exp_samples[0] if exp_samples else 'None'}")
